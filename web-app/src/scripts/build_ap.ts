...
try {
  ...
  (secp as any).utils.hmacSha256Sync = (
    key: Uint8Array,
    ...msgs: Uint8Array[]
...
function loadTinyDataset(csvPath: string): TinyRow[] {
  ...
  return table.slice(1, 6).map((r, i) => {
    ...
    if (!Number.isFinite(area))
      throw new Error(`Non-numeric area u redu ${i + 2}: ${r[idxArea]}`);
    if (!Number.isFinite(price))
      throw new Error(`Non-numeric price u redu ${i + 2}: ${r[idxPrice]}`);
    return { area, price };
  });
}
...
async function signCompact64(
  ...
  if (res && typeof res === "object") {
    if (typeof res.toCompactRawBytes === "function")
...
export async function build_ap() {
  try {
